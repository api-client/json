...
describe("RFC Examples", () => {
  ...
  it("Removing an Array Element", () => {
    ...
    const patch: JsonPatch = [{ op: "replace", path: "/baz", value: "boo" }];
    const expected = {
      baz: "boo",
      foo: "bar",
    };
  ...
  it("Moving a Value", () => {
    ...
    const patch: JsonPatch = [{ op: "move", from: "/foo/waldo", path: "/qux/thud" }];
    const expected = {
      foo: {
        bar: "baz",
      },
  ...
  it("Moving an Array Element", () => {
    ...
    const patch: JsonPatch = [{ op: "move", from: "/foo/1", path: "/foo/3" }];
  ...
  it("Testing a Value: Error", () => {
    const doc = { baz: "qux" };
    const patch: JsonPatch = [{ op: "test", path: "/baz", value: "bar" }];
    assert.throws(() => {
      apply(doc, patch);
    });
  });
  ...
  it("Adding a Nested Member Object", () => {
    const doc = { foo: "bar" };
    const patch: JsonPatch = [{ op: "add", path: "/child", value: { grandchild: {} } }];
    const expected = {
      foo: "bar",
      child: {
        grandchild: {},
  ...
  it("Ignoring Unrecognized Elements", () => {
    const doc = { foo: "bar" };
    // @ts-ignore
    const patch: JsonPatch = [{ op: "add", path: "/baz", value: "qux", xyz: 123 }];
    const expected = {
      foo: "bar",
      baz: "qux",
    };
  ...
  it("Adding to a Nonexistent Target", () => {
    const doc = { foo: "bar" };
    const patch: JsonPatch = [{ op: "add", path: "/baz/bat", value: "qux" }];
    assert.throws(() => {
      apply(doc, patch);
    });
  });
  ...
  it("Adding an Array Value", () => {
    const doc = { foo: ["bar"] };
    const patch: JsonPatch = [{ op: "add", path: "/foo/-", value: ["abc", "def"] }];
